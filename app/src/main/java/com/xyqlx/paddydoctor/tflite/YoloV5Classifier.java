...
package com.xyqlx.paddydoctor.tflite;
...
import com.xyqlx.paddydoctor.MainActivity;
import com.xyqlx.paddydoctor.env.Logger;
import com.xyqlx.paddydoctor.env.Utils;
...
public class YoloV5Classifier implements Classifier {
    ...
    public static YoloV5Classifier create(
        ...
        // 将label存储到YoloV5Classifier的labels数组中
        String line;
...
//        d.ANCHORS = anchors;
        ...
        // 这里有点bug，或许只有当classnum大于1时才能这么用，不然得不到正确的classnum
        // 也或许本就是错的，因此改成int numClass = shape[shape.length - 1]-5;
        // int numClass = shape[shape.length - 1];
        // 这个5代表xywh和conf，不能算在内
...
//        float[][][] outbuf = new float[1][output_box][labels.size() + 5];
        ...
        for (int i = 0; i < output_box; ++i){
            ...
            // if (confidenceInClass > getObjThresh() && detectedClass==0) {
